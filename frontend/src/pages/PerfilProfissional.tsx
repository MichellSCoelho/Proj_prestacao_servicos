...
export default function PerfilProfissional() {
  ...
  const ehProprioPerf = usuario?.tipo === 'profissional'
    ...
    <div style={{ background:'#0A0E1A', minHeight:'100vh', padding:'40px 48px' }}>
      {showModal && (
        <ModalSolicitacao
          ...
          profissionalNome={`${perfil.nome} ${perfil.sobrenome}`}
          categoria={perfil.categoria}
          precoHora={perfil.preco_hora}
        ...
        <div style={{ padding:'0 24px 40px' }}>
          <div style={{ display:'flex', alignItems:'flex-end', gap:16, marginBottom:20 }}>
            ...
            <div style={{ display:'flex', gap:10, paddingBottom:8, alignItems:'center' }}>
              {perfil.verificado && <div style={{ background:'rgba(59,130,246,0.15)', border:'1px solid rgba(59,130,246,0.4)', color:'#60a5fa', fontSize:12, fontWeight:700, padding:'4px 12px', borderRadius:50 }}>✓ Verificado</div>}
              {!ehProprioPerf && (
                <button onClick={() => setShowModal(true)} style={{ padding:'12px 28px', background:'#FF6B35', color:'#fff', border:'none', borderRadius:50, fontSize:15, fontWeight:700, cursor:'pointer', boxShadow:'0 0 24px rgba(255,107,53,0.35)' }}>
                  🔧 Contratar
                </button>
              )}
            </div>
          ...
          <div style={{ display:'flex', gap:32, marginBottom:24 }}>
            ...
            <div style={{ borderLeft:'1px solid rgba(255,255,255,0.07)', paddingLeft:32 }}><div style={{ fontWeight:800, fontSize:28, color:'#fff' }}>{perfil.total_servicos}</div><div style={{ color:'#8A95A8', fontSize:12 }}>serviços realizados</div></div>
          ...
          {/* Botão Contratar para mobile */}
          {!ehProprioPerf && (
            <button onClick={() => setShowModal(true)} style={{ width:'100%', padding:'14px', background:'#FF6B35', color:'#fff', border:'none', borderRadius:50, fontSize:15, fontWeight:700, cursor:'pointer', marginBottom:24, display:'block' }}>
              🔧 Contratar / Solicitar orçamento
            </button>
          )}
